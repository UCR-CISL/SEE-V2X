...
srslte_cell_sl_t cell_sl = {.nof_prb = 100, .tm = SRSLTE_SIDELINK_TM4, .cp = SRSLTE_CP_NORM, .N_sl_id = 0};
...
int main(int argc, char** argv)
{
  ...
  if (prog_args.log_file_name) {
    ...
    const char* subPathSci = "/v2x_pssch_ue_logfiles/sci";
    const char* subPathMac = "/v2x_pssch_ue_logfiles/mac";
    char*       pathSci    = (char*)malloc(strlen(home) + strlen(subPathSci) + 1);
    char*       pathMac    = (char*)malloc(strlen(home) + strlen(subPathMac) + 1);
    strcpy(pathSci, home);
    strcat(pathSci, subPathSci);
    strcpy(pathMac, home);
    strcat(pathMac, subPathMac);
    ...
    sprintf(pcap_filename,
            "%s/v2x_pcap_%jd.pcap",
            pathMac,
            (intmax_t)current_time);

    logfile   = fopen(filename, "w");
    pcap_file = LTE_PCAP_Open(MAC_LTE_DLT, pcap_filename);
  ...
  // write header
  fprintf(logfile, "rx_timestamp_us,priority,resource_reserv,freq_resource_localtion,time_gap,mcs_idx,retransmission,transmission_format,sf_idx,pssch_start_idx,nof_prb_pssch,N_x_id\n");
      ...
      for (uint32_t cyclic_shift = 0; cyclic_shift <= 9; cyclic_shift += 3) {
        ...
        if (srslte_pscch_decode(&pscch, equalized_sf_buffer, sci_rx, pscch_prb_start_idx) == SRSLTE_SUCCESS) {
          if (srslte_sci_format1_unpack(&sci, sci_rx) == SRSLTE_SUCCESS) {
            ...
            // Decode PSSCH
            uint32_t sub_channel_start_idx = 0; // Not used
            uint32_t L_subCH               = 0;
